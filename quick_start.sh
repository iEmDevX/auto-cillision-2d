...
do
    case $opt in
        ...
        "View Example JSON")
            ...
            python -c "import json; data=json.load(open('example/base.json')); print(json.dumps(data[:5], indent=2))"
            echo "..."
            echo ""
            echo "Total polygons: $(python -c "import json; print(len(json.load(open('example/base.json'))))")"
            break
            ;;
        "List Project Structure")
            echo ""
